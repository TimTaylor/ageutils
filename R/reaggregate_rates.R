reaggregate_rates <- function(
    ...
    if (length(rates) != length(bounds))
        stop("`rates` must be the same length as `lower_bound`.")
    ...
    # if (max(population_bounds) < max(bounds))
    ...
    # .reaggregate_rates(bounds, rates, new_bounds, population_bounds, population_weights)
    ...
    # calculate the combined upper and lower bounds
    all_lower <- sort(unique(c(bounds, population_bounds)))
    ...
    for (i in seq_along(old_container)) {
        ...
        # TODO - should we instead stick with?
        # if (all_upper[i] > old_upper[old_index])
        #     old_index <- old_index + 1L
        #
        # if (all_upper[i] > pop_upper[pop_index])
        #     pop_index <- pop_index + 1L
        #
        # if (all_upper[i] > new_upper[new_index])
